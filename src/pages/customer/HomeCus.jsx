...
// Mencegah render ulang kartu jika data produk tidak berubah secara fundamental.
const ProductCard = memo(({ product, categoryName }) => {
  return (
    <Link to={`/detail/${product.slug}`} className="group block border-4 border-black bg-white hover:-translate-y-2 transition-transform shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full min-w-[260px] md:min-w-[280px]">
      ...
      <div className="p-4 flex flex-col flex-grow justify-between bg-white">
        <div>
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            {product.category?.name || categoryName || 'Koleksi Riff'}
          </p>
        </div>
        <div className="mt-4 flex justify-between items-end border-t-2 border-gray-100 pt-3">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-black group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
...
export default function HomeCus() {
  ...
  // --- FETCH DATA TANPA CACHE ---
  useEffect(() => {
    let isMounted = true;

    const fetchHomeData = async () => {
      try {
        // Memanggil API yang kolomnya sudah kita "Diet" di Laravel (Tanpa Description)
        ...
        if (isMounted) {
          // Sinkronisasi dengan struktur Pagination Laravel
    ...
    <div className="bg-white">
      ...
      <section className="relative bg-black min-h-[80vh] flex items-center justify-center overflow-hidden border-b-8 border-black">
        ...
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none opacity-10">
          <h1 className="text-[20vw] font-black text-white whitespace-nowrap tracking-tighter">STREETWEAR</h1>
        </div>
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto flex flex-col items-center">
          <div className="bg-white border-4 border-black p-2 mb-6 shadow-[8px_8px_0px_0px_rgba(255,255,255,0.2)] transform -rotate-2">
            <span className="bg-black text-white px-4 py-1 text-sm font-black uppercase tracking-widest inline-block">Koleksi 2026</span>
          </div>
          <h1 className="text-6xl md:text-8xl font-black text-white tracking-tighter mb-6 uppercase leading-none drop-shadow-2xl">
            Riff.<br/><span className="text-transparent border-text-white italic">Fashion</span>
          </h1>
      ...
      <section className="border-b-8 border-black bg-gray-50">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 divide-y-4 md:divide-y-0 md:divide-x-4 divide-black border-x-4 border-black">
          {[
            { title: "Gratis Ongkir", desc: "Ke seluruh Indonesia*", icon: "M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" },
            { title: "100% Original", desc: "Kualitas premium terjamin", icon: "M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" },
            { title: "Tukar Ukuran", desc: "Garansi retur 7 hari", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" }
          ].map((feat, idx) => (
            <div key={idx} className="p-8 md:p-12 flex flex-col items-center text-center hover:bg-black hover:text-white transition-colors group">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mb-4 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d={feat.icon} />
              </svg>
              <h3 className="text-xl font-black uppercase tracking-tight mb-2">{feat.title}</h3>
              <p className="text-sm font-bold uppercase tracking-widest text-gray-500 group-hover:text-gray-300">{feat.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* 3. PRODUK TERBARU */}
      <section id="produk-terbaru" className="py-20 px-4 md:px-8 border-b-8 border-black">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12 border-b-4 border-black pb-4">
            <div>
              <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-1">Baru Mendarat</p>
              <h2 className="text-4xl md:text-5xl font-black text-black uppercase tracking-tighter italic">Produk Terbaru</h2>
            </div>
            <div className="flex space-x-2">
              <button onClick={() => slide('left')} className="p-3 border-4 border-black bg-white hover:bg-black hover:text-white transition shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none">
              ...
              <button onClick={() => slide('right')} className="p-3 border-4 border-black bg-white hover:bg-black hover:text-white transition shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none">
          ...
          <div ref={sliderRef} className="flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth pb-8 [&::-webkit-scrollbar]:hidden">
            {latestProducts.length > 0 ? (
              latestProducts.slice(0, 8).map((product) => (
                <div key={product.id} className="snap-start flex-none w-[80vw] sm:w-[300px]">
                  <ProductCard product={product} />
                </div>
              ))
            ) : (
              <div className="w-full text-center py-10 font-black uppercase text-gray-400">Belum ada produk baru.</div>
            )}
          </div>
        </div>
      </section>
      ...
      {categories.map((kategori) => (
        <section key={kategori.id} className="py-16 px-4 md:px-8 border-b-8 border-black bg-gray-50 even:bg-white">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {kategori.products && kategori.products.map((product) => (
                <ProductCard key={product.id} product={product} categoryName={kategori.name} />
              ))}
            </div>
          </div>
        </section>
      ...
      <section className="bg-black text-white py-24 px-4 text-center border-b-8 border-gray-900">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mb-6 text-white animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3" />
          </svg>
          <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-6">Explore Our Universe</h2>
          <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-10 max-w-xl">
            Dari T-shirt basic hingga celana cargo fungsional. Temukan seluruh identitas Riff Fashion di katalog lengkap kami.
          </p>
          <Link to="/katalog" className="bg-white text-black border-4 border-white px-12 py-6 text-xl font-black uppercase tracking-widest hover:bg-transparent hover:text-white transition-all shadow-[8px_8px_0px_0px_rgba(255,255,255,0.3)] hover:translate-y-1 hover:shadow-none">
            Semua Produk Riff.
          </Link>
        </div>
      </section>
