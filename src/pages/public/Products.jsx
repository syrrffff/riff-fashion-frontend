...
export default function Products() {
  ...
  const [categories, setCategories] = useState([]); // State untuk Dropdown Kategori
  ...
  const handleCategoryChange = (newCategory) => {
    setCurrentPage(1); // Selalu kembali ke halaman 1 saat ganti kategori
    if (newCategory) {
  ...
  const ProductCard = ({ product }) => (
    <Link to={`/product/${product.slug}`} className="group block border-4 border-black bg-white hover:-translate-y-2 transition-transform shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full">
      ...
      <div className="border-b-8 border-black pb-6 mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-sm font-bold text-gray-500 uppercase tracking-widest mt-2">
            Temukan gaya terbaikmu di sini.
          </p>
        </div>
        ...
        <div className="relative inline-block w-full md:w-auto mt-4 md:mt-0">
          ...
          {/* Ikon Panah Kustom agar terlihat lebih tajam */}
